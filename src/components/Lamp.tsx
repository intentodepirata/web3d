...
export const Lamp = () => {
  ...
  const [fixed] = useSphere(() => ({ args: [1], position: [-4, 8, -3], type: 'Static' }), useRef<Mesh>(null))
  const [lamp] = useBox(
    () => ({
      angulardamping: 1.99,
      args: [1, 0, 5],
  ...
  usePointToPointConstraint(fixed, lamp, { pivotA: [0, 0, 0], pivotB: [0, 3, 0] })
  ...
  return (
    <>
      <mesh ref={lamp} {...bind} scale={[0.4, 0.4, 0.4]}>
        ...
        <spotLight ref={light} position={[0, 10, -5]} angle={0.4} penumbra={1} intensity={0.3} castShadow />
      </mesh>
    </>
  )
}
